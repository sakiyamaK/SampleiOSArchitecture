...
final class RootViewController: UIViewController {
    enum SegueButton: String, CaseIterable {
        case mvc, mvp, mvvm01, mvvm02, mvvm03, viper01, viper02, viper03, viper99, shareDataViper
        ...
        func segue(from: UIViewController) {
            ...
            case SegueButton.viper01:
                Router.showVIPER01(from: from)
            case SegueButton.viper02:
                let vc = UIAlertController(title: "検索ワードを入れてね", message: "", preferredStyle: .alert)
                vc.addTextField()
                vc.addAction(UIAlertAction(title: "OK", style: .default) {_ in
                    let text = vc.textFields?.first?.text
                    Router.showVIPER02(from: from, depedency: .init(searchWord: text))
                })
                from.present(vc, animated: true)
            case SegueButton.viper03:
                Router.showVIPER03(from: from, dependencies: .init())
            case SegueButton.viper99:
                Router.showVIPER99(from: from)
